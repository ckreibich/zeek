##! This implements handling of peers that fall so far behind in message I/O
##! that the local Broker endpoint decides to unpeer them.  Zeek captures this
##! occurrence via logging (with a "peer-removed" entry in broker.log indicating
##! CAF's underlying reason, and a similar message in reporter.log), eventing
##! (via a Broker::peer_removed/lost event, also indicating the reason), and via
##! telemetry.
