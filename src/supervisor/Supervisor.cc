...
RecordValPtr Supervisor::NodeConfig::ToRecord() const {
    // Subtle: this can get called in middle of script parsing, but BiF state
    // initializes only after parsing has completed, so we can't use
    // BifType::Record::... here. To avoid using that shortcut incorrectly,
    // there is no BifType for Supervisor::NodeConfig.
